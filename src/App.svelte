  ...
  function nextAudio(pre) {
    ...
    if ("mediaSession" in navigator) {
      navigator.mediaSession.metadata = new MediaMetadata({
        ...
        artwork: [
          {
            src: "http://img.lanrentuku.com/img/allimg/1810/15409966139768.jpg",
            sizes: "192x192",
          },
        ],
      });
...
<div class="body">
  ...
  <audio
    id="audio_id"
    bind:this={audioDom}
    poster="http://img.lanrentuku.com/img/allimg/1810/15409966139768.jpg"
    controls
    autoplay
    on:ended={nextAudio}
    {src}>
  ...
  <h2 id="name">{name}</h2>
  <button type="button" class="btn btn-primary" on:click={nextAudio}>next</button>
</div>
